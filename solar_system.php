...
//		var oParticle = new THREE.Particle(new THREE.ColorFillMaterial(iColor, .4));
		...
		//oParticle.size = 50;
		oParticle.scale.x =oParticle.scale.y = Math.random() * 10 + 5;
		var t;
...
//					oParticle.material[0].color.setRGBA(r,g,b,Math.round(1000*fSpd/fParticleSpeed));
	...
	function init() {
		...
		for (var i = 0; i < iParticles; i++) {
			var oPoint = pointJet(0,0,0,(Math.random()-.5)*2,(Math.random()-.5)*2,5);
			oPoint.reset();
		...
		for (var i = 0; i < iParticles; i++) {
			...
			var oPoint = pointJet(xtemp,ytemp,0,0,0,0);
			oPoint.reset();
			particle = oPoint.getParticle();
			scene.addObject(particle);
